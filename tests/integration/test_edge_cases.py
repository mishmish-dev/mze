...
def test_arity_mismatch(tmp_path, clean_db):
    save_command("two-args", "diff {0} {1}")
    f1 = tmp_path / "f1.txt"
    f1.write_text("a")

    # Too few args
    with pytest.raises(SystemExit) as e:
        run_command("two-args", [str(f1)])
    ...
    f3.write_text("c")
    with pytest.raises(SystemExit) as e:
        run_command("two-args", [str(f1), str(f2), str(f3)])
    assert e.value.code == 1

def test_non_zero_exit_code(tmp_path, clean_db):
    save_command("fail-cmd", "false") # 'false' always returns 1
    f1 = tmp_path / "f1.txt"
    f1.write_text("a")

    with pytest.raises(SystemExit) as e:
        run_command("fail-cmd", [str(f1)])
    assert e.value.code == 1

def test_stderr_capture(tmp_path, clean_db):
    save_command("err-cmd", "echo 'error' >&2")
    f1 = tmp_path / "f1.txt"
    f1.write_text("a")
